...
ramsize	equ	$100000	; 1MB in size
...
; user stack
;
super_stack	equ	rambase+ramsize	; stack is at end of RAM
...
; to the main monitor code.
	...
	move.b	#$70,(PORTQS)	; 'p' ; PORTQS - port QS set to output #$70
	...
	move.w	#$92,($FFFF90)	; 'ó' ; channel 9 is output, capture TCR1, compare TCR1, don't change PAC, force pin high
...
gets:
	...
	; this routine always writes at LEAST 1 byte to the buffer
	; even in the event the passed buffer length is 0
	; calls getc which trashes other stuff (d0)
	
	subq.l	#1,d1	; subtract 1 FIRST because we need to reserve 1 location for the null
	bls	.done	; branch if result was <= 0
	jsr	getc
